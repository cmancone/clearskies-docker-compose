...
class BindingSpec(pinject.BindingSpec):
    ...
    def bind_item(cls, key, value):
        ...
        # what's up with this?  When we override the cursor backend, we no longer need the cursor to be provided
        # (since the cursor backend is already built, a cursor must have been either explicitly provided or over-ridden).
        # However, our provide_cursor_backend will still be called, and it requires a cursor, so pinject will still end
        # up implicitly trying to provide a cursor, which may be impossible for it and cause errors.  Therefore, if
        # we provide a manual cursor backend, also provide a "junk" cursor so that pinject won't try to build it.
        # Just make sure we don't also clobber a cursor that was otherwise provided already.
        if key == 'cursor_backend' and 'cursor' not in cls._class_bindings:
            cls._class_bindings['cursor'] = 'CURSOR_OVERRIDE'
